...
gpp::SoundInstanceWrapper* gpp::AudioComponent::createSoundInstance( const std::string& soundName, const std::string& path )
{
    ...
    GEP_ASSERT(m_soundInstances.exists(soundName) == false, "A soundInstance with the name provided alredy exists!", soundName.c_str(), m_pParentGameObject->getName().c_str())
...
{
    GEP_ASSERT(m_soundInstances.exists(soundName) == true, "A soundInstance with the name provided doesn't exist!", soundName.c_str(), m_pParentGameObject->getName().c_str())

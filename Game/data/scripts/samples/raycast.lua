...
function normalCamIsometricUpdate(updateData)
	...
	do -- ray cast
		...
		local distance = 100
		local ray      = dir:mulScalar(distance)
		rayIn.from     = player:getWorldPosition()
		rayIn.to       = rayIn.from + ray

		DebugRenderer:drawLine3D(rayIn.from, rayIn.to)

		local rayOut = world:castRay(rayIn)

		local message = "no hit detected."

		if rayOut:hasHit() then
			message = "hit detected: " .. rayOut.hitBody:getUserData():getName()
			local pointOfCollision = rayIn.from + ray:mulScalar(rayOut.hitFraction)
			local target = pointOfCollision + rayOut.normal:mulScalar(20)
			DebugRenderer:drawArrow(pointOfCollision, target)
		end

		DebugRenderer:printText(Vec2(0.0, -0.05), message)
	end
